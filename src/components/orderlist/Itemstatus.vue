...
<template>
	<div class="order_item_main">
		...
		<yd-infinitescroll :callback="loadList" ref="infinitescrollDemo" v-else-if="orders.length > 0">
						...
						<div class="layout">
							<ul>
								<li v-for="item in order.items">
									<router-link :to="'/order/o-'+ order.code">
										<div class="flexbox">
								...
								<div class="summary" v-if="order.totalPrice"><!--共<span v-for="item in order.items">{{item.quantity}}</span> 件商品--><b>小计:<span v-html="order.totalPrice"></span></b></div>
								<!-- 订单操作组件 -->
...
Vue.component(ListTheme.name, ListTheme);
	...
	export default {
		...
		methods: {
			...
			loadList() {
				Auth.then( auth=> {
					Ajaxy.get( `${configs.catShop.apiBaseUrl}orders-statused-${this.$route.params.status}?`, {take:this.pageSize, skip:this.page,} ,{token: auth.token} )
...
<style type="text/css">
	.order_item {border-top:1px solid #eee;}
	.order_item .order_zhuangt{padding:0 10px;height: 40px;line-height: 40px;background:#fff;}
	.order_item .order_zhuangt p{float: left}
	.order_item .order_zhuangt span{float: right;}
	.order_item  .layout > ul > li a{color:#000;}
	.order_item  .layout > ul > li .item.topalign {
		-webkit-box-align: flex-start;
		-webkit-align-items: flex-start;
		align-items: flex-start;
		background:#f9f9f9;
	}
	.order_item .m-popup-mask{background-color: rgba(0,0,0,.1);}
