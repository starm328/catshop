...
<div class="st-viewbox">
	<div class="address-add select-add">
		...
		<v-loged :needUser="true">
				...
				<mu-raised-button label="保存" class="demo-raised-button st-fixed fullHeight st-main-bgcolor" primary fullWidth backgroundColor="#FF4E4E" @click="submit"/>
			...
			submit(){
				...
				Auth.then( auth=> {
					Ajaxy.post( `${configs.catShop.apiBaseUrl}addresses`, this.add, { token: auth.token, } )
						...
						.when( 422, response=> {this.errors= response.json.errors;this.$dialog.loading.close();})
