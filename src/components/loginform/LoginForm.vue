...
<template>
	<div class="user-loginform">
		<div style="position:relative;width:100%;padding:0 10%;margin:30px auto 0  auto;background:#fff">
			<v-username v-if="username" @user="phonelogin"  @sign="signbox"/>
			<v-phone v-if="phone" @phone="usernamelogin"/>
			<v-sign v-if="sign" @phone="usernamelogin"/>
		</div>
...
import Loginphone from '@/components/loginform/Mobile';
import Loginusername from '@/components/loginform/Username';
import Sign from '@/components/loginform/Sign';
...
<style lang='less'>
.user-loginform{
	position: absolute;min-height: 100%;background: #fff;width: 100%;
	...
	.other{margin:10px 0;color:#999;line-height:30px;
		p{
			float:left;
		}
		span{float:right}
